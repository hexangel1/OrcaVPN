...
static struct orcavpn_server *create_server(const char *file)
{
	...
	const char *ip, *tun_name, *tun_addr, *tun_mask;
	...
	strcpy(serv->tun_mask, tun_mask);

	serv->tun_persist = (get_bool_var(config, "tun_persist") == 1);
	serv->private_ip = inet_network(serv->tun_addr);
	serv->private_mask = inet_network(serv->tun_mask);
	serv->port = port;
...
static int vpn_server_up(struct orcavpn_server *serv)
{
	...
	res = create_tun_if(serv->tun_name, serv->tun_persist);
	if (res < 0) {
	...
	res = setup_tun_if(serv->tun_name, serv->tun_addr, serv->tun_mask);
	if (res < 0) {
